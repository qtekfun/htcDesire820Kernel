...
int VP8GetHeaders(VP8Decoder* const dec, VP8Io* const io) {
  ...
  assert(headers.data_size >= headers.offset);  
  if (buf_size < 4) {
  ...
  if (!frm_hdr->key_frame_) {
    ...
    dec->buffer_flags_ = VP8Get(br) << 0;   
    dec->buffer_flags_ |= VP8Get(br) << 1;  
    ...
    dec->buffer_flags_ |= VP8Get(br) << 6;    
    dec->buffer_flags_ |= VP8Get(br) << 7;    
#else
    return VP8SetError(dec, VP8_STATUS_UNSUPPORTED_FEATURE,
                       "Not a key frame.");
#endif
  ...
  dec->buffer_flags_ |=
      (frm_hdr->key_frame_ || VP8Get(br)) << 8;    
#else
  VP8Get(br);   
#endif
