...
static errcode_t write_bitmaps(ext2_filsys fs, int do_inode, int do_block)
{
	dgrp_t 		i;
	unsigned int	j;
	int		block_nbytes, inode_nbytes, inodes_per_group;
	unsigned int	nbits;
	errcode_t	retval;
	...
	if (do_inode) {
		...
		inodes_per_group = EXT2_INODES_PER_GROUP(fs->super);
		retval = ext2fs_get_memalign(fs->blocksize, fs->blocksize,
					     &inode_buf);
		if (retval)
			return retval;
	...
	skip_block_bitmap:
		...
		retval = ext2fs_get_inode_bitmap_range(fs->inode_map,
				ino_itr, inodes_per_group, inode_buf);
		if (retval)
			return retval;
