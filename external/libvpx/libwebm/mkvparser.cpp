...
ContentEncoding::ContentCompression::ContentCompression()
    : algo(0),
      settings(NULL),
      //add by jinguo.wang 0108 begin
      size(0){
      //add by jinguo.wang 0108 end
}
...
//add by jinguo.wang 0108 begin
bool ContentEncoding::ParseContentCompressionEntry(
     long long start,
     long long size,
    IMkvReader* const pReader,
    ContentCompression* const compression) {

    if (pReader == NULL || compression == NULL) {
        return false;
    }
  ...
  while (pos < stop) {
    ...
    if (Match(pReader, pos, 0x1031, value)) {
      ...
      if (id == 0x1034) {
        ...
        if (ParseContentCompressionEntry(pos, size, pReader, compression)) {
...
int Track::Info::Copy(Info& dst) const
{
	...
	//add by maopei 0521 begin
	dst.codecPrivateOffset = codecPrivateOffset;
	//add by maopei 0521 end
    }
