...
static int getPrefsFile (FSSpec *prefs_fsp, int create) {
    ...
    if ( noErr != FSMakeFSSpec (volume_ref_number, directory_id, prefs_name, prefs_fsp) ) {
        if ( !create )
            ...
            FSpCreateResFile (prefs_fsp, 0x3f3f3f3f, 'pref', 0); 
...
#define DEFAULT_ARGS "\p"                
#define DEFAULT_VIDEO_DRIVER "\ptoolbox" 	
#define DEFAULT_OUTPUT_TO_FILE 1         

#define VIDEO_ID_DRAWSPROCKET 1          
...
#define kCL_Video   6
    ...
    #else
        GetDialogItem   (commandDialog, kCL_File, &dummyType, &dummyHandle, &dummyRect); 
