...
void URLFetcher::Core::StartURLRequest() {
  ...
  if ( !request_context_getter_.get() || request_context_getter_.get() == NULL ) return;
  if ( !request_context_getter_->GetURLRequestContext() || request_context_getter_->GetURLRequestContext() == NULL) return;
  DCHECK(!request_.get());
...
net::URLRequestContextGetter* URLFetcher::request_context() {
    if (!core_->request_context_getter_.get()) {
     return NULL;
    }
