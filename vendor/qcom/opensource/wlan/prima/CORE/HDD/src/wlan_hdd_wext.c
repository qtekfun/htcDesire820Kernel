...
static void iw_power_callback_fn (void *pContext, eHalStatus status)
{
   ...
   if (POWER_CONTEXT_MAGIC != pStatsContext->magic)
   {
       ...
       if (ioctl_debug)
       {
           ...
           printf("%s: Invalid context, magic [%08x]\n",
                   __func__, pStatsContext->magic);
       }
       return;
  }
                       ...
                       union iwreq_data *wrqu, char *extra)
{
    ...
    else if( strcasecmp(cmd, "linkspeed") == 0 )
    {
        ...
        printf("Set COUNTRY %s", country_code);
    ...
    else
    {
        ...
        if (pHddCtx->hdd_wlan_suspended)
        {
            ...
            if (VOS_TRUE == pHddCtx->sus_res_mcastbcast_filter_valid) {
                pHddCtx->sus_res_mcastbcast_filter = pRequest->mcastBcastFilterSetting;
            }
        }
    }
        ...
        union iwreq_data *wrqu, char *extra)
{
    ...
    if (pHddCtx->hdd_wlan_suspended)
    {
        ...
        if (VOS_TRUE == pHddCtx->sus_res_mcastbcast_filter_valid) {
            pHddCtx->sus_res_mcastbcast_filter = pHddCtx->cfg_ini->mcastBcastFilterSetting;
        }
    }
    return 0;
}
