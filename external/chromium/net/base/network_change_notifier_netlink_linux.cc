    ...
    switch (netlink_message_type) {
      ...
      case RTM_NEWADDR:
        if (IsIPv6Update(netlink_message_header) &&
          ...
          return false;
#ifdef CFLAG_PROJECT_CMCC
        LOG(INFO) << "ignore RTM_NEWADDR from netlink socket to avoid ERR_ABORTED of changed ip address";
        return false;
#else
        return true;
#endif
      case RTM_DELADDR:
#ifdef CFLAG_PROJECT_CMCC
        LOG(INFO) << "ignore RTM_DELADDR from netlink socket to avoid ERR_ABORTED of changed ip address";
        return false;
#else
        return true;
#endif
