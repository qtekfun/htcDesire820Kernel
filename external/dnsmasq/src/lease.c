...
void lease_update_file(time_t now)
{
  ...
  if (file_dirty != 0 && daemon->lease_stream)
      ...
      for (lease = leases; lease; lease = lease->next)
	{
	  ...
	  if (!lease->hostname) ourprintf(&err, "Device-%.2X%.2X%.2X ", lease->hwaddr[lease->hwaddr_len - 3], lease->hwaddr[lease->hwaddr_len - 2], lease->hwaddr[lease->hwaddr_len - 1]);
          else ourprintf(&err, "%s ", lease->hostname);
