...
#ifdef	INCLUDE_3C503
static unsigned char	t503_output;	
#endif
	...
	const char *p)			
{
        ...
        if (!(eth_flags & FLAG_PIO)) {
                memcpy((char *)eth_bmem, d, ETH_ALEN);	
                memcpy((char *)eth_bmem+ETH_ALEN, nic->node_addr, ETH_ALEN); 
                *((char *)eth_bmem+12) = t>>8;		
                *((char *)eth_bmem+13) = t;
...
#endif
{
	...
	outb(0x80, eth_asic_base + WD_MSR);	
	for (i=0; i<ETH_ALEN; i++) {
