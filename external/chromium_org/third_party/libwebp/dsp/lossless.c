  ...
  for (mode = 0; mode < kNumPredModes; ++mode) {
    ...
    for (y = 0; y < ymax; ++y) {
      ...
      for (x = 0; x < xmax; ++x) {
        ...
        if (row == 0) {
          predict = (col == 0) ? ARGB_BLACK : current_row[col - 1];  
        } else if (col == 0) {
...
void VP8LSubtractGreenFromBlueAndRed(uint32_t* argb_data, int num_pixs) {
  ...
  for (; i + 4 < num_pixs; i += 4) {
    ...
    const __m128i in_00g0 = _mm_and_si128(in, mask);     
    const __m128i in_0g00 = _mm_slli_epi32(in_00g0, 8);  
    const __m128i in_000g = _mm_srli_epi32(in_00g0, 8);  
