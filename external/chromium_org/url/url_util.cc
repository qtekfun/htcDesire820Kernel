...
// Mutex to prevent concurrent access of standard_schemes when
// multi_threaded_message_loop is enabled. The vulnerable period is after
// calling CefInitialize (which starts multiple threads running) and
// before calling LockStandardSchemes (if it is ever called).
base::Lock standard_schemes_lock;
...
void InitStandardSchemes() {
  base::AutoLock auto_lock(standard_schemes_lock);
  if (standard_schemes)
    return;
