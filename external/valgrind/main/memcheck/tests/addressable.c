...
static void test1()
{
	...
	VALGRIND_CHECK_MEM_IS_DEFINED(m, pgsz*5); 
}
...
static void test3()
{
	...
	VALGRIND_CHECK_MEM_IS_DEFINED(&m[pgsz], pgsz); 
}
...
static void test4()
{
	...
	VALGRIND_CHECK_MEM_IS_DEFINED(m, pgsz); 
	m[44] = 'y';		
...
static void test5()
{
	...
	memset(m, 'x', 10);
	VALGRIND_CHECK_MEM_IS_DEFINED(m, 10);	
	VALGRIND_CHECK_MEM_IS_DEFINED(m+10, 10); 

	mprotect(m, pgsz*5, PROT_NONE);
	mprotect(m, pgsz*5, PROT_READ);

	VALGRIND_CHECK_MEM_IS_DEFINED(m, 10);	
	VALGRIND_CHECK_MEM_IS_DEFINED(m+20, 10); 
}
