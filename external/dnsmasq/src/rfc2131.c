...
#define DISABLE_DNSMASQ_PRI_LOG 1
		  ...
		  size_t sz, time_t now, int unicast_dest, int *is_inform)
{
...
#if DISABLE_DNSMASQ_PRI_LOG
    my_syslog(MS_DHCP | LOG_INFO, _("%u client provides name: ***"), ntohl(mess->xid));
#else
  ...
  if (have_config(config, CONFIG_NAME))
      ...
      if (strlen(client_hostname) != 0)
	{
      ...
      sprintf(mac_name, "Device-%.2X%.2X%.2X", emac[emac_len - 3], emac[emac_len - 2], emac[emac_len - 1]);
      hostname = mac_name;
      domain = "Test";
      hostname_auth = 1;
...
#if DISABLE_DNSMASQ_PRI_LOG
    my_syslog(MS_DHCP | LOG_WARNING, _("Ignoring domain *** for DHCP host name ***"));
#else
  ...
  if (context)
    {
      if (!option_find2(netid, config_opts, OPTION_NETMASK))
        ...
        {
            my_syslog(MS_DHCP | LOG_INFO, "option_put NETMASK %s", inet_ntoa(context->netmask));
            if(context->netmask.s_addr == 0) {
                inet_aton("255.255.255.0", &(context->netmask));
                my_syslog(MS_DHCP | LOG_INFO, "force update NETMASK to %s", inet_ntoa(context->netmask));
            }
	...
	option_put(mess, end, OPTION_ROUTER, INADDRSZ, ntohl(context->router.s_addr));
	  ...
	  !option_find2(netid, config_opts, OPTION_DNSSERVER))
	    {
    	    if (!(context->flags & CONTEXT_NO_DNS))
