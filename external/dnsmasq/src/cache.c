...
#ifdef FRISBEE_FEATURE_SUPPORT
static int frisbee_mode_fake_enable = 1;
#endif
...
struct crec *cache_find_by_name(struct crec *crecp, char *name, time_t now, unsigned short prot)
{
  struct crec *ans;
#ifdef FRISBEE_FEATURE_SUPPORT
  char frisbee_mode[20]={'\0'};

  property_get(FRISBEE_ENABLE, frisbee_mode, "");
  if (strcmp(frisbee_mode, "1") == 0) {
    if(frisbee_mode_fake_enable  == 1) {
      if (strncmp(name, FRISBEE_SITE, strlen(FRISBEE_SITE)) != 0) {
         my_syslog(LOG_INFO, _("frisbee_mode:%s,ori name:%s(len=%d/fri_len=%d),fake=%d"), frisbee_mode, name,strlen(name),strlen(FRISBEE_SITE),frisbee_mode_fake_enable);
         memset(name,'\0', (strlen(FRISBEE_SITE)+1));
         strncpy(name, FRISBEE_SITE, strlen(FRISBEE_SITE));
         my_syslog(LOG_INFO, _("frisbee_mode:%s,mod name:%s(len=%d/fri_len=%d),fake=%d"), frisbee_mode, name,strlen(name),strlen(FRISBEE_SITE),frisbee_mode_fake_enable);
      }
    }
  }
#endif
			    ...
			    unsigned short flags, int index, int addr_dup)
{
  ...
  property_get(FRISBEE_ENABLE, frisbee_mode, "");
  if (strcmp(frisbee_mode, "1") == 0) {
    my_syslog(LOG_INFO, _("add_hosts_entry:FRISBEE MODE is %s,frisbee_mode_fake_enable=%d"),frisbee_mode,frisbee_mode_fake_enable);
  }
#endif
  ...
  if (lookup && (lookup->flags & F_HOSTS))
    {
      ...
      if (memcmp(&lookup->addr.addr, addr, addrlen) == 0)
        {
          my_syslog(LOG_INFO, _("add_hosts_entry:Remove duplicates in hosts files...cache->name.sname:%s"),cache->name.sname);
          free(cache);
          return;
        }
    }
