...
Bool VG_(am_is_valid_for_client)( Addr start, SizeT len, 
                                  UInt prot )
{
   return is_valid_for_client( start, len, prot, False );
}

Bool VG_(am_is_valid_for_client_or_free_or_resvn)
   ( Addr start, SizeT len, UInt prot )
{
   return is_valid_for_client( start, len, prot, True );
}
...
# else
   aspacem_minAddr = (Addr) 0x100000000;  
...
# endif
   ...
   aspacem_minAddr = (Addr) 0x04000000; 

#  if VG_WORDSIZE == 8
     aspacem_maxAddr = (Addr)0x800000000 - 1; 
#    ifdef ENABLE_INNER
...
Bool VG_(am_extend_map_client)( Bool* need_discard,
