...
Time CanonExpirationInternal(const CookieMonster::ParsedCookie& pc,
  ...
  if (pc.HasExpires()) {
    ...
    return current + (CookieMonster::ParseCookieTime(pc.Expires()) - server_time);
  }
...
Time CanonExpiration(const CookieMonster::ParsedCookie& pc,
  ...
  Time expiration_time = CanonExpirationInternal(pc, current, server_time);
...
bool CookieMonster::SetCookieWithCreationTimeAndOptions(
  ...
  scoped_ptr<CanonicalCookie> cc;
  Time cookie_expires = CanonExpiration(pc, creation_time, server_time, options);
...
CookieMonster::CanonicalCookie::CanonicalCookie(const GURL& url,
  ...
  if (has_expires_)
    expiry_date_ = CanonExpiration(pc, creation_date_, creation_date_, CookieOptions());
